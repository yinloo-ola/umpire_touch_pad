...
# Plan 2.2: Phase 1 Store Tests

## Objective
Write a comprehensive Vitest unit test suite for all `matchStore.js` doubles logic from Phase 1. Cover every behavioural requirement: quadrant swaps, rotation formula, deuce handling, singles guard, umpire overrides, between-game mandatory receiver, deciding-game swap (singles and doubles), and double-trigger prevention.

## Context
- `frontend/src/stores/matchStore.js` — the store under test (Plan 1 implementation)
- Pinia test pattern: `setActivePinia(createPinia())` in `beforeEach`, then `useMatchStore()`
- Store uses `(state) =>` getters — access via store instance in tests

### Test Fixture — Doubles Match Object
```js
const doublesMatch = {
  type: 'doubles',
  bestOf: 5,
...
### Score Helper
To simulate scoring without worrying about `isStarted`/`pointStarted`:
```js
function scorePoints(store, p1pts, p2pts) {
  // Force-set scores directly for rotation testing (bypasses flow guards)
  store.p1Score = p1pts
  store.p2Score = p2pts
}
```
For full `handleScore()` flow tests (deciding-game swap), call `store.startPoint()` + `store.handleScore(player, 1)` in a loop.

## Tasks

<task type="auto">
  <name>Create test file with Plan 1.1 tests: quadrant swaps and getters</name>
  <files>frontend/src/stores/__tests__/matchStore.doubles.test.js</files>
  <action>
    Create `frontend/src/stores/__tests__/matchStore.doubles.test.js` with:
  ...
  <verify>ls frontend/src/stores/__tests__/matchStore.doubles.test.js</verify>
  <done>File exists (ls returns the path)</done>
</task>

<task type="auto">
  <name>Run test suite and confirm all tests pass</name>
  <files>frontend/src/stores/__tests__/matchStore.doubles.test.js</files>
  <action>
    Run:
    ```bash
  ...
  <verify>cd frontend && npm run test 2>&1 | tail -5</verify>
  <done>Output contains "Tests" with 0 "failed" tests. All test counts green.</done>
</task>

## Success Criteria
- [ ] Test file exists at `frontend/src/stores/__tests__/matchStore.doubles.test.js`
- [ ] `npm run test` in `frontend/` passes with ≥ 30 tests, 0 failures
- [ ] `make test` from root also passes
